...
public class ContaEspecialModel extends ContaModel{
    
    public ContaEspecialModel(){
    }
    
    public ContaEspecialModel(int id,String conta, int idCliente, Enum tipoDeConta, double limite) throws Exception {
        super(id, conta, idCliente, tipoDeConta, limite);
    }
      
     @Override
    public boolean saque(double valor) throws Exception {
        return ((valor > super.getSaldo() + super.getLimite()) ? false : super.saque(valor));
    }
    
     public ContaEspecialModel (String dadosTxtConta) throws Exception {
